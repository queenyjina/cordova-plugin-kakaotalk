...
public class KakaoTalk extends CordovaPlugin {
	 ...
	 * @param callbackContext
	 */
	private void session(final CallbackContext callbackContext)
	{
		cordova.getActivity().runOnUiThread(new Runnable() {
			@Override
			public void run() {
	...
	private JSONObject handleResult(UserProfile userProfile)
	{
		...
		try {
			response.put("accessToken", Session.getCurrentSession().getAccessToken());
	...
	private class SessionCallback implements ISessionCallback {
		...
		public void onSessionOpenFailed(KakaoException exception) {
			if(exception != null) {
				...
				callbackContext.error("kakao : closed!");
			}
		}
	}
